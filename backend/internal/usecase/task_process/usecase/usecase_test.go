...
func TestUsecase_Handle_Success(t *testing.T) {
	...
	tests := []struct {
		name  string
		setup func(taskRepo *MocktaskRepository, versionGetService *MockversionGetService, variableProcessService *MockvariableProcessService, dataProcessService *MockdataProcessService)
	}{
		{
			name: "Success",
			setup: func(taskRepo *MocktaskRepository, versionGetService *MockversionGetService, variableProcessService *MockvariableProcessService, dataProcessService *MockdataProcessService) {
				...
				taskUpdate = domain.TaskUpdate{ID: taskID, Status: task_domain.StatusSucceed, Result: result}
				taskRepo.EXPECT().UpdateByID(ctx, taskUpdate).Return(nil)
			},
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.setup(taskRepo, versionGetService, variableProcessService, dataProcessService)

			usecase := New(taskRepo, versionGetService, variableProcessService, dataProcessService)
	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(taskRepo *MocktaskRepository, versionGetService *MockversionGetService, variableProcessService *MockvariableProcessService, dataProcessService *MockdataProcessService) {
				...
				taskRepo.EXPECT().UpdateByID(ctx, gomock.Any()).Return(errors.New("test6"))
			},
			want: "test6",
		},
		{
