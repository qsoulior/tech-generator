...
func (s *repositorySuite) TestRepository_DeleteByID() {
	...
	// handle
	err = repo.DeleteByID(ctx, projectID)
	require.NoError(s.T(), err)

	projects, err := test_db.SelectEntitiesByID[test_db.Project](s.C(), "project", []int64{projectID})
	require.NoError(s.T(), err)
	require.Empty(s.T(), projects)

	templates, err := test_db.SelectEntitiesByID[test_db.Project](s.C(), "template", []int64{templateID})
	require.NoError(s.T(), err)
